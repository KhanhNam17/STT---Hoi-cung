# build_metadata_sliding_window.py
# ============================================================
# Hướng 2: Gộp tất cả lượt nói của mỗi speaker thành 1 WAV dài
# Whisper dùng sliding window 30s để xử lý toàn bộ
#
# So sánh với Hướng 1:
#   Hướng 1: Cắt thành nhiều segment nhỏ (5-50s) → nhiều WAV files
#   Hướng 2: Gộp toàn bộ lượt nói của mỗi speaker → 1 WAV/speaker
#            → Whisper tự sliding window → context tốt hơn
#
# Cách dùng:
#   python build_metadata_sliding_window.py
#
# Output:
#   data/podcast2_sliding/
#   ├── Host_Thuy_Minh.wav        ← concat tất cả turns của Host
#   ├── Khanh_Vy.wav              ← concat tất cả turns của Khánh Vy
#   └── metadata_sliding.csv      ← 1 row per speaker
# ============================================================
...
TRANSCRIPT_TXT  = "podcast_2.mp3.txt"           # file .txt có timestamp
FULL_WAV        = "data/podcast2/podcast_2.wav"  # file WAV đã convert
OUTPUT_DIR      = "data/podcast2_sliding"        # thư mục output
...
# Có gộp im lặng giữa các turns không?
# True  → concat trực tiếp (không có khoảng lặng giữa turns)
# False → giữ nguyên khoảng lặng (realistic hơn, nhưng WAV dài hơn)
STRIP_SILENCE   = True

# Thêm padding (giây) đầu/cuối mỗi turn để tránh cắt sát
